...
// If the difference in X value is between 2 and 5
if (abs(xDiff) >= 2 and abs(xDiff) < 5)
{
...
// If the difference in X value is over 5
else if (abs(xDiff) >= 5)
{
	// Use the spriteList_BaseTilt2 array (heavy tilt)
	arrayToUse = spriteList_BaseTilt2;
	// If the difference in X is greater than 0, flip the x scale to -1
	if (xDiff > 0) spriteScale = -1;
}
...
for (var i = 0; i < array_length(arrayToUse); i++)
{
	...
	var totalShipDisplayHeight = 500 * array_length(arrayToUse);
	...
	var shipY = -150 - ((shipYSeparation + shipYMovementSpeed) % totalShipDisplayHeight);
